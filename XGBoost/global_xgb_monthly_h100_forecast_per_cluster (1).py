...
python3 global_xgb_monthly_h100_forecast_per_cluster.py \
...
def make_macro_features(cfg: Config, month_dates: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    if "cpi" in f.columns:
        ...
        f["cpi_yoy_12m"] = f["cpi"].pct_change(52)
        f["cpi_acceleration_4m"] = f["cpi_mom_4m"].diff(4)

    if "m2" in f.columns:
        f["m2_mom_4m"] = f["m2"].pct_change(4)
        f["m2_yoy_12m"] = f["m2"].pct_change(52)

    if {"m2_yoy_12m", "cpi_yoy_12m"}.issubset(f.columns):
        f["m2_real_growth_12m"] = f["m2_yoy_12m"] - f["cpi_yoy_12m"]

    if "unrate" in f.columns:
        f["unrate_diff_4m"] = f["unrate"].diff(4)
        f["unrate_diff_12m"] = f["unrate"].diff(13)
    ...
    for spread in [
        ...
        if spread in f.columns:
            ...
            f[f"{spread}_roll_mean_12m"] = f[spread].rolling(13, min_periods=4).mean()
            f[f"{spread}_roll_std_12m"] = f[spread].rolling(13, min_periods=4).std()
...
def add_rank_features(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["ret_m", "ret_roll_std_12m", "ret_mom_13m", "month_volume", "price_feature"]:
...
def run_optuna(
    ...
    valid_max = min(cfg.max_optuna_rows // 3, max(1000, len(valid_pairs) * len(horizons)))
...
def train_final(
    ...
    bst.save_model(out_dir / f"global_xgb_monthly_model_{model_name}.json")
...
def parse_cluster_ids_arg(cluster_ids: Optional[str]) -> Optional[List[int]]:
    if cluster_ids is None or str(cluster_ids).strip() == "":
        return None
    out = []
    for x in str(cluster_ids).split(","):
        x = x.strip()
        if not x:
            continue
        out.append(int(x))
...
def available_cluster_ids(panel: pd.DataFrame, cfg: Config) -> List[int]:
    if "cluster_id" not in panel.columns:
        raise RuntimeError("В panel нет cluster_id. Для per-cluster версии нужен --cluster-csv.")

    explicit = parse_cluster_ids_arg(cfg.cluster_ids)
    if explicit is not None:
        return explicit

    ids = sorted(pd.Series(panel["cluster_id"].dropna().unique()).astype(int).tolist())
    ids = [cid for cid in ids if cid != -999]
    if not cfg.include_noise_cluster:
        ids = [cid for cid in ids if cid != -1]
    return ids


def count_target_pairs(
    df: pd.DataFrame,
    horizons: List[int],
    target_before: Optional[pd.Timestamp] = None,
    target_from: Optional[pd.Timestamp] = None,
    ...
    for h in horizons:
        ...
        if tcol not in df.columns or dcol not in df.columns:
            continue
        sub = df[[tcol, dcol]].dropna().copy()
        if sub.empty:
            continue
        d = pd.to_datetime(sub[dcol])
        mask = pd.Series(True, index=sub.index)
        if target_before is not None:
            mask &= d < target_before
        if target_from is not None:
            mask &= d >= target_from
        if target_to is not None:
            mask &= d <= target_to
...
def cluster_readiness_report(
    ...
    for cid in cluster_ids:
        ...
        rows.append({
            ...
            "train_pairs_short": count_target_pairs(cp, short_horizons, target_before=train_target_before),
            "valid_pairs_short": count_target_pairs(cp, short_horizons, target_from=valid_target_from, target_to=valid_target_to),
            "test_pairs_short": count_target_pairs(cp, short_horizons, target_from=eval_start, target_to=eval_end),
            "train_pairs_long": count_target_pairs(cp, long_horizons, target_before=train_target_before),
            "valid_pairs_long": count_target_pairs(cp, long_horizons, target_from=valid_target_from, target_to=valid_target_to),
            "test_pairs_long": count_target_pairs(cp, long_horizons, target_from=eval_start, target_to=eval_end),
            "train_pairs_all": count_target_pairs(cp, all_horizons, target_before=train_target_before),
            "valid_pairs_all": count_target_pairs(cp, all_horizons, target_from=valid_target_from, target_to=valid_target_to),
            "test_pairs_all": count_target_pairs(cp, all_horizons, target_from=eval_start, target_to=eval_end),
        })
    rep = pd.DataFrame(rows).sort_values("cluster_id").reset_index(drop=True)
    rep.to_csv(out_dir / "cluster_readiness_report.csv", index=False)
    print("[CLUSTERS] readiness:")
    print(rep.to_string(index=False))
    return rep


def aggregate_backtest_outputs(all_meta: List[pd.DataFrame], cfg: Config, out_dir: Path) -> pd.DataFrame:
    if not all_meta:
        raise RuntimeError("Нет результатов backtest по кластерам для агрегации.")

    meta = pd.concat(all_meta, ignore_index=True)
    meta = meta.sort_values(["cluster_id", "target_month", "Ticker", "horizon_months"]).reset_index(drop=True)

    meta.to_csv(out_dir / "backtest_last_year_monthly_predictions.csv", index=False)

    by_h = []
    for h, g in meta.groupby("horizon_months"):
        by_h.append({"horizon_months": int(h), **price_metrics(g["actual_price"].values, g["pred_price"].values)})
    metrics_h = pd.DataFrame(by_h).sort_values("horizon_months")
    ...
    by_cluster = []
    for cid, g in meta.groupby("cluster_id"):
        row = {"cluster_id": int(cid), "tickers": int(g["Ticker"].nunique())}
        row.update(price_metrics(g["actual_price"].values, g["pred_price"].values))
        by_cluster.append(row)
    metrics_cluster = pd.DataFrame(by_cluster).sort_values("cluster_id")

    by_cluster_group = []
    for (cid, group), g in meta.groupby(["cluster_id", "model_group"]):
        row = {"cluster_id": int(cid), "model_group": group, "tickers": int(g["Ticker"].nunique())}
        row.update(price_metrics(g["actual_price"].values, g["pred_price"].values))
        by_cluster_group.append(row)
    metrics_cluster_group = pd.DataFrame(by_cluster_group).sort_values(["cluster_id", "model_group"])
    ...
    by_ticker = []
    for tkr, g in meta.groupby("Ticker"):
        row = {"Ticker": tkr, "cluster_id": int(g["cluster_id"].iloc[0])}
        row.update(price_metrics(g["actual_price"].values, g["pred_price"].values))
        by_ticker.append(row)
    metrics_t = pd.DataFrame(by_ticker).sort_values(["cluster_id", "WAPE"])

    overall = {
        "eval_end": cfg.eval_end_date,
        "eval_years": int(cfg.eval_years),
        "mode": "monthly_per_cluster_two_models",
        "n_clusters_trained": int(meta["cluster_id"].nunique()),
        "short_horizon_months": int(cfg.short_horizon_months),
        "short_model": "cluster_specific_short_macro",
        "long_model": "cluster_specific_long_no_macro",
        **price_metrics(meta["actual_price"].values, meta["pred_price"].values),
    }

    metrics_h.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_horizon.csv", index=False)
    metrics_group.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_model_group.csv", index=False)
    metrics_cluster.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_cluster.csv", index=False)
    metrics_cluster_group.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_cluster_and_model_group.csv", index=False)
    metrics_m.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_month.csv", index=False)
    metrics_t.to_csv(out_dir / "backtest_last_year_monthly_metrics_by_ticker.csv", index=False)
    ...
    print("[AGG BACKTEST]", overall)
    return meta


def aggregate_future_forecasts(all_forecasts: List[pd.DataFrame], out_dir: Path) -> Optional[pd.DataFrame]:
    if not all_forecasts:
        return None
    out = pd.concat(all_forecasts, ignore_index=True)
    out = out.sort_values(["cluster_id", "Ticker", "horizon_months"]).reset_index(drop=True)
    out.to_csv(out_dir / "forecast_monthly_next_year.csv", index=False)
    out.pivot(index="forecast_month", columns="Ticker", values="pred_price").to_csv(
        out_dir / "forecast_monthly_next_year_wide_prices.csv"
    )
...
def parse_args() -> Config:
    ap = argparse.ArgumentParser("Per-cluster XGBoost monthly stock forecast optimized for H100")
    ...
    ap.add_argument("--cluster-ids", default=None, help="Например: '0,1,2'. Если не задано, берутся все кластеры кроме -999.")
...
def main() -> None:
    ...
    print("=" * 80)
    print("[RUN] Per-cluster XGBoost MONTHLY forecast")
    ...
    # На всякий случай убираем year, если он попал из старой версии/данных.
    feat_cols = [c for c in feat_cols if c != "year"]
    ...
    cluster_ids = available_cluster_ids(panel, cfg)
    if not cluster_ids:
        raise RuntimeError("Не найдено ни одного cluster_id для обучения.")

    readiness = cluster_readiness_report(
        panel=panel,
        cfg=cfg,
    ...
    trained_rows = []
    all_backtest_meta: List[pd.DataFrame] = []
    ...
    for _, r in readiness.iterrows():
        ...
        print("\n" + "=" * 80)
        print(f"[CLUSTER {cid}] start, tickers={tickers}")
        print("=" * 80)

        if int(r["train_pairs_short"]) < cfg.min_train_pairs_per_cluster or int(r["valid_pairs_short"]) < cfg.min_valid_pairs_per_cluster or int(r["test_pairs_short"]) < cfg.min_test_pairs_per_cluster:
            print(f"[CLUSTER {cid}] skip: too few short pairs")
            trained_rows.append({"cluster_id": cid, "status": "skipped_short_pairs", **r.to_dict()})
            continue

        if long_horizons and (int(r["train_pairs_long"]) < cfg.min_train_pairs_per_cluster or int(r["valid_pairs_long"]) < cfg.min_valid_pairs_per_cluster or int(r["test_pairs_long"]) < cfg.min_test_pairs_per_cluster):
            print(f"[CLUSTER {cid}] skip: too few long pairs")
            trained_rows.append({"cluster_id": cid, "status": "skipped_long_pairs", **r.to_dict()})
            continue

        cdir = out_dir / f"cluster_{cid}"
        cdir.mkdir(parents=True, exist_ok=True)

        cp = panel[panel["cluster_id"].astype(int) == cid].copy()
        cp = cp.sort_values(["Ticker", "month_date"]).reset_index(drop=True)

        with open(cdir / "cluster_info.json", "w", encoding="utf-8") as f:
            json.dump({
                "cluster_id": cid,
                "tickers": sorted(cp["Ticker"].unique().tolist()),
                "n_tickers": int(cp["Ticker"].nunique()),
                "panel_rows": int(len(cp)),
                "readiness": {k: (int(v) if isinstance(v, (np.integer, int)) else float(v) if isinstance(v, (np.floating, float)) else v) for k, v in r.to_dict().items()},
        ...
        best_short = run_optuna(
            ...
            model_name=f"cluster_{cid}_short_macro",
        )
        ...
        if long_horizons:
            best_long = run_optuna(
                ...
                model_name=f"cluster_{cid}_long_no_macro",
            )
        ...
        meta = evaluate_last_year_backtest(
            bst_short=bst_short,
            bst_long=bst_long,
            panel=cp,
            short_feat_cols=short_feat_cols,
        ...
        trained_rows.append({"cluster_id": cid, "status": "trained", **r.to_dict()})

        del cp, bst_short, bst_long
        gc.collect()

    train_report = pd.DataFrame(trained_rows)
    train_report.to_csv(out_dir / "cluster_training_report.csv", index=False)

    aggregate_backtest_outputs(all_backtest_meta, cfg, out_dir)
    aggregate_future_forecasts(all_forecasts, out_dir)

    print("\n[DONE]")
    print("Results:", out_dir.resolve())
    print("Cluster report:", (out_dir / "cluster_training_report.csv").resolve())
