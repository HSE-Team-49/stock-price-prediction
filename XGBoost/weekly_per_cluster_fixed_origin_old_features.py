...
class Config:
    ...
    min_test_pairs_per_cluster: int = 2_000
...
def make_macro_features(cfg: Config, week_dates: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    for fname, name in MACRO_FILES.items():
        ...
        base[f"{name}_age_available_days"] = pd.Series(week_dates - pd.to_datetime(avail_aligned)).dt.days.astype(float).to_numpy()
        base[f"{name}_age_observation_days"] = pd.Series(week_dates - pd.to_datetime(obs_aligned)).dt.days.astype(float).to_numpy()
...
def aggregate_backtest_outputs(all_meta: List[pd.DataFrame], cfg: Config, out_dir: Path) -> pd.DataFrame:
    ...
    meta = meta.sort_values(["cluster_id", "target_week", "Ticker", "horizon_weeks"]).reset_index(drop=True)

    meta.to_csv(out_dir / "backtest_last_year_weekly_predictions.csv", index=False)

    by_h = []
    for h, g in meta.groupby("horizon_weeks"):
        by_h.append({"horizon_weeks": int(h), **price_metrics(g["actual_price"].values, g["pred_price"].values)})
    metrics_h = pd.DataFrame(by_h).sort_values("horizon_weeks")
    ...
    metrics_h.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_horizon.csv", index=False)
    metrics_group.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_model_group.csv", index=False)
    metrics_cluster.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_cluster.csv", index=False)
    metrics_cluster_group.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_cluster_and_model_group.csv", index=False)
    metrics_w.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_week.csv", index=False)
    metrics_t.to_csv(out_dir / "backtest_last_year_weekly_metrics_by_ticker.csv", index=False)
...
def aggregate_future_forecasts(all_forecasts: List[pd.DataFrame], out_dir: Path) -> Optional[pd.DataFrame]:
    ...
    out = out.sort_values(["cluster_id", "Ticker", "horizon_weeks"]).reset_index(drop=True)
    out.to_csv(out_dir / "forecast_weekly_next_year.csv", index=False)
    out.pivot(index="forecast_week", columns="Ticker", values="pred_price").to_csv(
        out_dir / "forecast_weekly_next_year_wide_prices.csv"
    )
...
def evaluate_fixed_origin_forecast(
    ...
    """Fixed-origin weekly forecast на последнем cfg.eval_years году."""
    eval_end = pd.Timestamp(cfg.eval_end_date)
    eval_start = eval_end - pd.DateOffset(years=int(cfg.eval_years))
...
def parse_args() -> Config:
    ...
    ap.add_argument("--min-train-pairs-per-cluster", type=int, default=20000)
    ap.add_argument("--min-valid-pairs-per-cluster", type=int, default=2000)
    ap.add_argument("--min-test-pairs-per-cluster", type=int, default=2000)
...
def main() -> None:
    ...
    print("=" * 80)
    print("[RUN] Per-cluster XGBoost WEEKLY forecast")
    ...
    for _, r in readiness.iterrows():
        ...
        cp = cp.sort_values(["Ticker", "week_date"]).reset_index(drop=True)
