    ...
    if cfg.use_gpu:
        try:
            return xgb.QuantileDMatrix(
                ...
                missing=np.nan,
                nthread=cfg.n_jobs,
                max_bin=int(best_params.get("max_bin", 256)),
            )
        except Exception as e:
            print(f"[WARN] QuantileDMatrix final train failed, fallback to DMatrix: {e}", flush=True)
...
def make_xgb_matrix(
    ...
    if cfg.use_gpu:
        try:
            ...
            print(f"[WARN] QuantileDMatrix failed, fallback to DMatrix: {e}", flush=True)
...
def run_optuna(
    ...
    Xtr, ytr, _ = stack_horizons(train, feat_cols, horizons, optuna_max_rows, cfg.random_state)

    valid_max = None if (not cfg.max_optuna_rows or cfg.max_optuna_rows <= 0) else min(cfg.max_optuna_rows // 3, max(1000, len(valid) * len(horizons)))
...
def train_final(
    ...
    model_path = out_dir / f"global_xgb_daily_model_{model_name}.json"
    ...
    dtr = make_final_train_matrix(X, y, best["best_params"], cfg)
...
def evaluate_fixed_origin_forecast(
    ...
    eval_end = pd.Timestamp(cfg.eval_end_date)
    eval_start = eval_end - pd.DateOffset(months=int(cfg.eval_months))
    origin_date = pd.Timestamp(panel.loc[pd.to_datetime(panel["date"]) < eval_start, "date"].max())
    if pd.isna(origin_date):
        raise RuntimeError(f"Не найден origin_date < {eval_start.date()}")

    origin = (
        panel[pd.to_datetime(panel["date"]) == origin_date]
        .sort_values(["Ticker", "date"])
        .drop_duplicates("Ticker", keep="last")
        .copy()
    )
    if origin.empty:
        raise RuntimeError("origin rows пустые")
    ...
    def collect_predict(horizons: List[int], feat_cols: List[str], bst: xgb.Booster, model_group: str):
        ...
        for h in horizons:
            ...
            x = sub[feat_cols].copy()
            x["horizon"] = np.float32(h)
            m = sub[["date", "Ticker", "price", pcol, dcol]].copy()
            rename = {"date": "origin_date", "price": "origin_price", pcol: "actual_price", dcol: "target_date"}
    ...
    if long_horizons and bst_long is not None:
        ...
        if m is not None:
            metas.append(m)
    if not metas:
        raise RuntimeError("Не удалось собрать fixed-origin выборку")
    ...
    meta.to_csv(out_dir / "fixed_origin_daily_predictions.csv", index=False)
    metrics_h.to_csv(out_dir / "fixed_origin_daily_metrics_by_horizon.csv", index=False)
    metrics_group.to_csv(out_dir / "fixed_origin_daily_metrics_by_model_group.csv", index=False)
    metrics_d.to_csv(out_dir / "fixed_origin_daily_metrics_by_date.csv", index=False)
    metrics_t.to_csv(out_dir / "fixed_origin_daily_metrics_by_ticker.csv", index=False)
    with open(out_dir / "fixed_origin_daily_metrics_overall.json", "w", encoding="utf-8") as f:
        json.dump(overall, f, ensure_ascii=False, indent=2)
    print("[FIXED ORIGIN] daily:", overall)
    return meta
...
def main() -> None:
    ...
    print("=" * 80)
    print("[RUN] Global XGBoost daily forecast")
    ...
    panel_path = out_dir / "forecast_panel_features.parquet"
    feature_sets_path = out_dir / "feature_sets_short_macro_long_no_macro.json"

    if cfg.resume_run_dir and panel_path.exists() and feature_sets_path.exists():
        print("[RESUME] loading existing panel:", panel_path)
        panel = pd.read_parquet(panel_path)

        with open(feature_sets_path, "r", encoding="utf-8") as f:
            feature_sets = json.load(f)

        short_feat_cols = feature_sets["short_features"]
        long_feat_cols = feature_sets["long_features"]
        macro_excluded = feature_sets.get("macro_features_excluded_from_long", [])
        short_horizons, long_horizons = horizon_groups(cfg)

        print(f"[RESUME] panel rows={len(panel):,}, cols={len(panel.columns):,}")
        print(f"[RESUME] short_features={len(short_feat_cols):,}, long_features={len(long_feat_cols):,}")
    else:
    ...
    target_cols = [f"target_logret_h{h}" for h in range(1, cfg.forecast_horizon + 1)]
    has_target = panel[target_cols].notna().any(axis=1)
    ...
    historical = panel[(panel["date"] < train_target_cutoff) & has_target].copy()

    if historical.empty:
        raise RuntimeError(
            "historical пустой. Проверь eval_end_date/eval_months/forecast_horizon "
            "или наличие данных до тестового окна."
        )
    ...
    print(f"[SPLIT] optuna cutoff={cutoff.date()}, train={len(train):,}, valid={len(valid):,}")

    if train.empty or valid.empty:
        raise RuntimeError("Пустой train или valid. Проверь даты/горизонт/данные.")
    ...
    short_model_path = out_dir / "global_xgb_daily_model_short_macro.json"
    if cfg.resume_run_dir and short_model_path.exists():
        print("[RESUME] loading existing short model:", short_model_path)
        bst_short = xgb.Booster()
        bst_short.load_model(str(short_model_path))
    else:
        best_short = run_optuna(train, valid, short_feat_cols, cfg, out_dir, short_horizons, "short_macro")
        bst_short = train_final(final_train, short_feat_cols, best_short, cfg, out_dir, short_horizons, "short_macro")

    bst_long = None
    if long_horizons:
        long_model_path = out_dir / "global_xgb_daily_model_long_no_macro.json"
        if cfg.resume_run_dir and long_model_path.exists():
            print("[RESUME] loading existing long model:", long_model_path)
            bst_long = xgb.Booster()
            bst_long.load_model(str(long_model_path))
        else:
            best_long = run_optuna(train, valid, long_feat_cols, cfg, out_dir, long_horizons, "long_no_macro")
            bst_long = train_final(final_train, long_feat_cols, best_long, cfg, out_dir, long_horizons, "long_no_macro")

    validate(bst_short, bst_long, valid, short_feat_cols, long_feat_cols, cfg, out_dir)

    evaluate_fixed_origin_forecast(bst_short, bst_long, panel, short_feat_cols, long_feat_cols, cfg, out_dir)

    if cfg.save_future_forecast:
        forecast_future(bst_short, bst_long, panel, short_feat_cols, long_feat_cols, cfg, out_dir)
